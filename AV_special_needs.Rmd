...
```
```{r}
# control for demographics, helpful, prof, skill, service, med, work, alt
...
```{r}
# demographics only
summary(model <- glm(wtr ~ as.factor(gender)+age+income+as.factor(emp)+as.factor(racecat)+ as.factor(blind)+as.factor(deaf)+ as.factor(mobility)+as.factor(psy)+as.factor(otherdis), data = d, family = binomial(logit)))
list(McFadden=mcfadden(model), CoxSnell=coxsnell(model), Nagelkerke=nagelkerke(model))
```
```{r}
summary(model <- glm(wtr ~ as.factor(gender)+age+income+as.factor(emp)+as.factor(racecat)+as.factor(dis)+as.factor(alt), data = d, family = binomial(logit)))
```
