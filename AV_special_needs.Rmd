...
##concern_human
```{r}
#not willing to ride because of prefer human driver
d <- d %>% mutate(concern_human=ifelse(concern_code_more==6, 1, 0)) 
d$concern_human[is.na(d$concern_code_more) == TRUE] <- NA
#altenatively code: Cost Concerns, job loss, lack of human interaction, societal impact, etc
#d <- d %>% mutate(concern_human=ifelse(concern_code_td==4, 1, 0)) 
#d$concern_human[is.na(d$concern_code_td) == TRUE] <- NA
```
```{r}
d %>% group_by(concern_human) %>% count()
```
...
```
```{r}
# control for demographics, test helpful, prof, skill, service, med, work, alt
summary(model <- glm(wtr ~ as.factor(gender)+age+income+as.factor(emp)+as.factor(racecat)+as.factor(dis)+alt, data = d, family = binomial(logit)))
list(McFadden=mcfadden(model), CoxSnell=coxsnell(model), Nagelkerke=nagelkerke(model))
```
```{r}
# demographics only
summary(model <- glm(wtr ~ as.factor(gender)+age+income+as.factor(emp)+as.factor(racecat)+as.factor(dis), data = d, family = binomial(logit)))
list(McFadden=mcfadden(model), CoxSnell=coxsnell(model), Nagelkerke=nagelkerke(model))
```

```{r}
# demographics only (seperate disabilities)
summary(model <- glm(wtr ~ as.factor(gender)+senior+income+as.factor(emp)+as.factor(racecat)+ as.factor(blind)+as.factor(deaf)+ as.factor(mobility)+as.factor(psy)+as.factor(otherdis), data = d, family = binomial(logit)))
list(McFadden=mcfadden(model), CoxSnell=coxsnell(model), Nagelkerke=nagelkerke(model))
```

```{r}
# disability & helpful interaction: nothing
summary(model <- glm(wtr ~ as.factor(gender)+age+income+as.factor(emp)+as.factor(racecat)+ as.factor(dis)*helpful, data = d, family = binomial(logit)))
list(McFadden=mcfadden(model), CoxSnell=coxsnell(model), Nagelkerke=nagelkerke(model))
```

```{r}
# replace age with senior
# senior & helpful interaction: nothing
summary(model <- glm(wtr ~as.factor(gender)+age*medrely +income+as.factor(emp)+as.factor(racecat)+as.factor(dis), data = d, family = binomial(logit)))
list(McFadden=mcfadden(model), CoxSnell=coxsnell(model), Nagelkerke=nagelkerke(model))
...
##helpfulness vs dis
```{r}
d %>% filter(!is.na(wtr)) %>% group_by(dis,helpful) %>% summarise(pct_willing=mean(wtr)*100)
```

##dis

```{r}
d %>% filter(!is.na(wtr)) %>% group_by(dis) %>% summarise(n=n(),pct_willing=mean(wtr)*100)
```

##race
```{r}
d %>% filter(!is.na(wtr)) %>% group_by(racecat) %>% summarise(n=n(),pct_willing=mean(wtr)*100)
```
# try extra
...
#d %>% filter(!is.na(wtr)) %>% filter(!is.na(dis))%>% filter(dis!=1) %>% group_by(alt) %>% summarise(n=n(),pct_willing=mean(wtr)*100)  # the same thing
d %>% filter(!is.na(wtr)) %>% filter(accom==1) %>% group_by(alt) %>% summarise(n=n(),pct_willing=mean(wtr)*100) 
```
```{r}
# only people with dis or older than 65
sn <- d %>% filter(accom==1|senior==1)
```

```{r}
summary(model <- glm(wtr ~as.factor(senior) + as.factor(dis)+as.factor(gender)+income+ as.factor(emp)+as.factor(racecat), data = sn, family = binomial(logit)))
list(McFadden=mcfadden(model), CoxSnell=coxsnell(model), Nagelkerke=nagelkerke(model))
round(exp(coef(model)), 2)
```
## only people older than 65
```{r}
old <- d %>% filter(senior==1)
```

```{r}
summary(model <- glm(wtr ~ as.factor(dis)+as.factor(gender)+income+ as.factor(emp)+as.factor(racecat), data = old, family = binomial(logit)))
list(McFadden=mcfadden(model), CoxSnell=coxsnell(model), Nagelkerke=nagelkerke(model))
round(exp(coef(model)), 2)
```
...
```{r}
## if special needs populations are more heavily dependent upon public transportation
summary(model <- glm(alt ~ as.factor(dis) + age +as.factor(gender) +income+ as.factor(emp)+as.factor(racecat), data = d, family = binomial(logit)))
list(McFadden=mcfadden(model), CoxSnell=coxsnell(model), Nagelkerke=nagelkerke(model))
round(exp(coef(model)), 2)
```
#-------------
...
summary(model <- glm(concern_human ~ as.factor(dis) + age +as.factor(gender) +income+ as.factor(emp)+as.factor(racecat), data = d, family = binomial(logit)))
list(McFadden=mcfadden(model), CoxSnell=coxsnell(model), Nagelkerke=nagelkerke(model))
round(exp(coef(model)), 2)
```
```{r}
d %>% filter(!is.na(concern_human)) %>% group_by(dis) %>% summarize(n=n(), mean(concern_human))
```

```{r}
# riders with mobility disabilities are more likely to have negative feelings about AVs
summary(model<- polr(as.factor(feel_senti) ~ as.factor(dis) + age +as.factor(gender) +income+ as.factor(emp)+as.factor(racecat), Hess=T, data=d))
...
```
```{r}
d %>% filter(!is.na(feel_code_td)) %>% filter(!is.na(dis)) %>% ggplot()+
  geom_histogram(aes(x=feel_code_td), binwidth = 0.8)+
  facet_wrap(~dis)
```
```{r}
d %>% filter(!is.na(feel_code_td)) %>% filter(!is.na(dis)) %>% filter(feel_code_td!=0) %>%ggplot()+
  geom_histogram(aes(x=dis, fill=as.factor(feel_code_td)), position="fill",binwidth = 1)+
